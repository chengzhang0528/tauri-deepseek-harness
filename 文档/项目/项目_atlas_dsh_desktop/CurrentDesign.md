...
主窗口标题显示本轮 Harness 版本；窗口顶部仅提供文件、视图、帮助，托盘共用动作。文件提供关闭窗口和单一退出；视图提供显示主窗口和 F11 全屏；帮助提供检查更新、软件更新和关于。下载及安装位于软件更新，来源、指定版本、诊断、恢复及停止后台服务位于高级选项。关于只显示产品与版本信息，内部退出证据只在诊断信息中显示。查看信息不下载。准备阶段使用原生 TaskDialog 和菜单状态，关闭提示不取消已串行化操作。
...
计划切换遇到存活树会保留运行并说明缺口。单一退出入口在退出证据缺失时提示任务可能未完成，用户取消则保留运行，明确确认后强制停止全树再退出。关窗隐藏至托盘，运行继续。用户可另选明确强制停止/强制退出；强制终止 Job 后须观察 ActiveProcesses 为零才清除运行。强制停止不被记录为正常收尾。
