...
`official.rs` 接官方元数据、Node ZIP 和 npm；`runtime.rs` 管副本及双指针；`process.rs` 管一次启动、带认证参数的端点及 bridge；`job.rs` 管所属进程集合；`host.rs` 管原生交互与唯一调度；`dialogs.rs` 管原生反馈。没有自有 Web 页面、更新服务或业务镜像。
...
主窗口标题显示本轮 Harness 版本；窗口顶部仅提供文件、视图、帮助，托盘共用动作。文件提供关闭窗口和单一退出；视图提供显示主窗口和 F11 全屏；帮助直接提供检查更新、立即更新、软件更新和关于。“立即更新”复用已下载副本或准备所选具体版本，再确认重启；手动检查发现更新时直接询问是否更新。下载及安装也可从软件更新进入，来源、指定版本、诊断、恢复及停止后台服务位于高级选项。关于只显示产品与版本信息，内部退出证据只在诊断信息中显示。查看信息不下载。准备阶段使用原生 TaskDialog 和菜单状态，关闭提示不取消已串行化操作。
...
立即更新准备完成后绑定具体副本；存活运行缺少退出证据时说明中断任务的风险，用户取消则保留运行和下载，明确选择强制重启更新后才结束全树并激活。单一退出入口在退出证据缺失时提示任务可能未完成，用户取消则保留运行，明确确认后强制停止全树再退出。关窗隐藏至托盘，运行继续。用户可另选明确强制停止/强制退出；强制终止 Job 后须观察 ActiveProcesses 为零才清除运行。强制停止不被记录为正常收尾。
