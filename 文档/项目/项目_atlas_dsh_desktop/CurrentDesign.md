...
主窗口标题显示本轮 Harness 版本；窗口和托盘共享版本与更新、检查、关于、下载、确认切换、重开/同版恢复及退出动作。查看信息不下载。准备阶段使用原生 TaskDialog 和菜单状态，关闭提示不取消已串行化操作。
...
当前正常退出与计划切换遇到存活树会保留运行并说明缺口。关窗隐藏至托盘，运行继续。用户可另选明确强制停止/强制退出；强制终止 Job 后须观察 ActiveProcesses 为零才清除运行。强制停止不被记录为正常收尾。
