...
4. 宿主运行期间自动检查并下载、安装准备、暂存合格更新；不增加退出后继续运行的服务。“帮助 → 检查更新”是唯一手动更新入口，第一次点击只检查并展示当前运行版本、来源、固定版本及检查结果。原生结果对话框仅在有具体目标时提供“下载更新”或“重启并更新”；无更新不显示安装操作，失败可重新检查。下载后另行确认具体版本与重启风险；取消保留当前运行和已下载更新。检查、下载、暂存及固定版本偏好均不代表切换确认；未确认的 staged 不因宿主重开而自动启用。来源失败不得宣称已是最新，固定版本和仅本机检查须明确说明范围。
5. 卸载默认删除安装器和运行时拥有的文件，保留 Harness 配置、会话、工作区登记和用户结果；删除用户数据必须是单独的明确选择。
6. 打开应用即可识别当前 Harness 版本，并从主窗口找到更新入口。窗口标题显示运行版本，原生菜单采用“文件、视图、帮助”，托盘共用动作。帮助提供检查更新、更新设置、故障处理及关于；没有重复更新动作或深层软件更新菜单，不以禁用菜单项承载长状态文字。更新设置使用原生对话框，显示当前来源与版本偏好，选择列表标记当前值；保存不下载或重启。故障处理独立提供诊断信息、启动或修复当前版本、强制停止应用；运行中禁用修复，无运行时禁用强制停止，进行中的串行操作禁用重复入口。窗口重新显示由视图菜单承担。“关于”只展示产品和版本信息。无运行显示未运行，版本不可读显示未知。
...
9. 计划更新先完成正常退出；无法取得正常退出证据时保留运行，只有用户明确确认风险并选择强制重启更新才终止全树。确认只适用于具体副本和发行，背景发现不得替换该目标。目标有效且无未结束运行时才切换；新运行就绪且页面呈现成功后才报告新版本可用。current 已切换但启动失败须如实报告并进入前向修复。
