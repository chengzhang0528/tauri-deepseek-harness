...
4. 宿主运行期间自动检查并下载、安装准备、暂存合格更新；不增加退出后继续运行的服务。用户可以立即检查、下载具体目标和确认切换。“立即更新”串起下载、准备和具体版本的重启确认；检查发现更新时直接提供操作，已下载状态给出明确入口。检查、下载、暂存及固定版本偏好均不代表切换确认；未确认的 staged 不因宿主重开而自动启用。来源选择独立于自动触发，来源失败不解释为“已是最新”。
5. 卸载默认删除安装器和运行时拥有的文件，保留 Harness 配置、会话、工作区登记和用户结果；删除用户数据必须是单独的明确选择。
6. 打开应用即可识别当前 Harness 版本，并从主窗口找到查看与更新入口。采用窗口标题展示运行版本，窗口原生菜单采用“文件、视图、帮助”：文件提供关闭窗口和退出，视图提供显示主窗口和全屏，帮助提供检查更新、立即更新、软件更新及关于；托盘共用动作。版本来源、指定版本和诊断仅放在软件更新的高级选项。“关于”只展示客户端与 Harness 版本及产品说明；当前版本来自本轮运行，选择目标和已下载目标分别标注。无运行显示未运行，版本不可读显示未知。查看信息不触发下载或切换。
