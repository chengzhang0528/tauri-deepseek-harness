# DSH Desktop 工作空间结构与知识放置契约

Status: Active
Kind: StructureContract
Scope: atlas-dsh-desktop / 目录与知识放置
Owner: 项目维护者
Updated: 2026-09-22
Depends On:
- ../AGENTS.md

本文是 DSH Desktop 工作空间目录职责、文档 Kind 和知识放置的唯一当前契约。项目事实由 `项目/项目_atlas_dsh_desktop/` 下的正式所有者维护。
...
| `src-tauri/` | DSH Desktop Rust 宿主、桌面 bridge 与安装器配置 | 文档治理控制面和活动计划 |
| `scripts/` | 构建、发布工具与定向测试 | 产品规则与任务过程记录 |

新增项目时，先验证项目与源码位置，再在本契约同一改动中声明其精确文档位置；只创建已有唯一事实的类别目录。
