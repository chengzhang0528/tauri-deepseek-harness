...
The project-configured immutable source is the client-visible origin for workspace-owned first-install and launcher-managed artifacts unless the ProductContract explicitly enables a multi-source runtime. In that case, each enabled provider keeps its official discovery/acquisition owner and every candidate still requires the same manifest, integrity, provenance, compatibility, and health admission. Package-manager/store/framework-managed components continue to use their established official owner; do not duplicate that owner's update implementation merely because an Installer exists.
...
Do not add an undeclared mirror, fallback origin, or second permanent binary store. A single-source contract blocks publication, first install, or update when its configured source is unavailable; a multi-source contract may use another explicitly enabled provider but never GitHub or a registry merely as an emergency fallback. Fixed endpoints remain deployment configuration; application code must not add custom host, scheme, certificate, Origin/Host, redirect, or source-allowlist enforcement. A Bootstrap compatibility change is an Installer/Launcher release, even when the visible Manager change is small.
