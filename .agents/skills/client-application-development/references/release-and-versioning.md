...
For a replaceable managed runtime, multiple configured providers are the default client-visible candidate set; a ProductContract may explicitly narrow it to one provider. Each enabled provider keeps its official discovery/acquisition owner and every candidate still requires the same manifest, integrity, provenance, compatibility, and health admission. Workspace-owned installer artifacts retain their configured immutable source, while package-manager/store/framework-managed components continue to use their established official owner; do not duplicate that owner's update implementation merely because an Installer exists.
...
Do not add an undeclared mirror, fallback origin, or second permanent binary store. The default multi-source policy may use another configured provider when one is unavailable; a single-source contract blocks publication, first install, or update when its configured source is unavailable. Never use GitHub or a registry merely as an emergency fallback. Fixed endpoints remain deployment configuration; application code must not add custom host, scheme, certificate, Origin/Host, redirect, or source-allowlist enforcement. A Bootstrap compatibility change is an Installer/Launcher release, even when the visible Manager change is small.
