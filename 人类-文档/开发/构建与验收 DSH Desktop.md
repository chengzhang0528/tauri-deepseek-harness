...
首次无运行时会直接通过官方发行方式准备私有 Node 与精确 Harness npm 版本；系统 Node 不作为产品运行时。已有 current 时保留原版启动，后台只准备 staged。“版本与更新”可查看、检查、下载及确认具体目标；查看不会下载。

关窗保留运行，托盘可重开。当前上游没有完整退出证据，正常退出会说明缺口；需停止时明确选择强制停止或强制退出。进程树结束后才能恢复或切换。详细边界见[产品契约](../../文档/项目/项目_atlas_dsh_desktop/ProductContract.md)。
